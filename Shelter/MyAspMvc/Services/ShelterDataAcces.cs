...
namespace MyAspMvc
{
    ...
    public class ShelterDataAccess : IShelterDataAccess
    {
        ...
        {
            var entityType = dbContext.Model.FindEntityType(typeof(Animal));

            var cats = _context.Model.FindEntityType(typeof(Animal));
            return _context.Animals.Where(x => x is Cat).ToList();
